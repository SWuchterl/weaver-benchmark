...
def pairwise_lv_fts(xi, xj, num_outputs=4, eps=1e-8, for_onnx=False):
    ...
    if num_outputs > 4:
        lnds2 = torch.log(torch.clamp(-to_m2(xi - xj, eps=1e-8), min=eps))
        # print ("\n lnds2 mean \n",lnds2.mean())
        # print ("\n lnds2 max \n",lnds2.max())
...
class ParticleTransformerTagger(nn.Module):
    ...
    def __init__(self,
        ...
        self.lep_trimmer = SequenceTrimmer(enabled=trim and not for_inference)

        self.pf_embed = Embed(pf_input_dim, embed_dims, activation=activation)
        self.sv_embed = Embed(sv_input_dim, embed_dims, activation=activation)
        self.lep_embed = Embed(lep_input_dim, embed_dims, activation=activation)
    ...
    def forward(self, pf_x, pf_v=None, pf_mask=None, sv_x=None, sv_v=None, sv_mask=None, lep_x=None, lep_v=None, lep_mask=None):
        # x: (N, C, P)
        # v: (N, 4, P) [px,py,pz,energy]
        # mask: (N, 1, P) -- real particle = 1, padded = 0
        ...
        with torch.no_grad():
            ...
            lep_x, lep_v, lep_mask, _ = self.lep_trimmer(lep_x, lep_v, lep_mask)
            v = torch.cat([pf_v, sv_v, lep_v], dim=2)
            mask = torch.cat([pf_mask, sv_mask, lep_mask], dim=2)

        with torch.cuda.amp.autocast(enabled=self.use_amp):
            pf_x = self.pf_embed(pf_x)  # after embed: (seq_len, batch, embed_dim)
            sv_x = self.sv_embed(sv_x)
            lep_x = self.lep_embed(lep_x)
            x = torch.cat([pf_x, sv_x, lep_x], dim=0)
